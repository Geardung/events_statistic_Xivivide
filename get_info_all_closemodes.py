...
def main():
    text = ""
    eventers_info_text = "Статистика клозмодеров: "
    ...
    for eventer in all_eventers:
        ...
        for event in eventer_events:
            if event.eventEx_id.type != "close": continue
        ...
        if a: eventers_info_text += f' \n\nКлоузмод {eventers_info[-1]["eventer_tag"]}\nВсего клозов за период: {eventers_info[-1]["count_events"]}\nСуммарная длительность всех клозов: {eventers_info[-1]["summary_duration"]} минут\nВсего кругов за все клоузы: {eventers_info[-1]["summary_krugs"]}\nВсего выдано серверной валюты: {eventers_info[-1]["summary_prizes"]}\nВсего получено баллов: {eventers_info[-1]["summary_points"]}'
